import React, { useEffect, useState } from 'react'
import { useParams, useNavigate } from '@tanstack/react-router'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { ChevronRight, Lock, CheckCircle2, Play, ArrowLeft, BookOpen } from 'lucide-react'
import { toast } from 'sonner'
import { LessonApiService } from './LessonApiService'
import { SubtopicApiService } from '@/features/subtopics/SubtopicApiService'
import type { Lesson } from './types'
import type { Subtopic } from '@/features/subtopics/types'

const LessonsPage: React.FC = () => {
  const { subtopicId } = useParams({ from: '/dashboard/lessons/$subtopicId' })
  const navigate = useNavigate()
  const [lessons, setLessons] = useState<Lesson[]>([])
  const [subtopic, setSubtopic] = useState<Subtopic | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetchData()
  }, [subtopicId])

  const fetchData = async () => {
    try {
      setLoading(true)
      const subtopicData = await SubtopicApiService.findById(subtopicId)
      setSubtopic(subtopicData)
      
      const lessonsData = await LessonApiService.findBySubtopicId(subtopicId, subtopicData.topicId)
      setLessons(lessonsData.sort((a, b) => a.order - b.order))
    } catch (error) {
      console.error('Error fetching data:', error)
      toast.error('Error al cargar las lecciones')
    } finally {
      setLoading(false)
    }
  }

  const handleLessonClick = (lessonId: string) => {
    navigate({ to: `/dashboard/lesson/${lessonId}/exercise` })
  }

  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Cargando lecciones...</p>
        </div>
      </div>
    )
  }

  return (
    <div className="max-w-4xl mx-auto">
      {/* Back Button */}
      <Button
        variant="ghost"
        className="mb-4"
        onClick={() => navigate({ to: `/dashboard/subtopics/${subtopic?.topicId}` })}
      >
        <ArrowLeft size={16} className="mr-2" />
        Volver a subtópicos
      </Button>

      {/* Subtopic Header */}
      {subtopic && (
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{subtopic.name}</h1>
          <p className="text-gray-600">{subtopic.description}</p>
        </div>
      )}

      {/* Lessons List */}
      <div className="space-y-4">
        {lessons.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              <BookOpen className="mx-auto h-16 w-16 text-gray-400 mb-4" />
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                No hay lecciones disponibles
              </h3>
              <p className="text-gray-500">
                Este subtópico aún no tiene lecciones configuradas
              </p>
            </CardContent>
          </Card>
        ) : (
          lessons.map((lesson, index) => {
            const isFirst = index === 0
            const isLocked = !isFirst
            const isCompleted = false

            return (
              <Card
                key={lesson.id}
                className={`transition-all ${
                  isLocked 
                    ? 'opacity-60 cursor-not-allowed' 
                    : 'hover:shadow-lg cursor-pointer'
                }`}
                onClick={() => !isLocked && handleLessonClick(lesson.id)}
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                          isCompleted 
                            ? 'bg-green-100' 
                            : isLocked 
                            ? 'bg-gray-200' 
                            : 'bg-blue-100'
                        }`}>
                          {isCompleted ? (
                            <CheckCircle2 className="text-green-600" size={24} />
                          ) : isLocked ? (
                            <Lock className="text-gray-400" size={24} />
                          ) : (
                            <Play className="text-blue-600" size={24} />
                          )}
                        </div>
                        <div>
                          <CardTitle className="text-xl">
                            Lección {index + 1}: {lesson.title}
                          </CardTitle>
                          <Badge variant={isCompleted ? 'success' : isLocked ? 'outline' : 'default'}>
                            {isCompleted ? 'Completado' : isLocked ? 'Bloqueado' : 'Comenzar'}
                          </Badge>
                        </div>
                      </div>
                      <CardDescription className="ml-15 line-clamp-2">
                        {lesson.content.substring(0, 150)}...
                      </CardDescription>
                    </div>
                    {!isLocked && (
                      <ChevronRight className="text-gray-400 flex-shrink-0" size={24} />
                    )}
                  </div>
                </CardHeader>
              </Card>
            )
          })
        )}
      </div>
    </div>
  )
}

export default LessonsPage
